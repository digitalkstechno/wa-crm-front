...
export default function SuperAdminModal({ mode, entity, onClose, onSaved, showToast }: any) {
  const [formData, setFormData] = useState({
    fullName: '', email: '', phone: '', password: '', status: 'Active'
  });
  ...
  useEffect(() => {
    if (mode === 'edit' && entity) {
      ...
      setFormData({ fullName: '', email: '', phone: '+91 ', password: '', status: 'Active' });
    }
  }, [mode, entity]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: any = { ...formData, roleType: 'Super Admin' };
      ...
      showToast('success', `Super Admin ${mode === 'create' ? 'created' : 'updated'} successfully`);
      onSaved();
    } catch (err: any) {
      showToast('error', err.message || 'Failed to save super admin');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-xl font-bold text-gray-900">{mode === 'create' ? 'Add Super Admin' : 'Edit Super Admin'}</h3>
