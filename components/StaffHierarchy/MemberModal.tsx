...
export default function MemberModal({ mode, entity, parentOpts, teams, managers, onClose, onSaved, showToast }: any) {
  const [formData, setFormData] = useState({
    fullName: '', email: '', phone: '', password: '',
    teamId: parentOpts?.teamId || '',
    managerId: parentOpts?.managerId || '',
    status: 'Active',
    firmId: parentOpts?.firmId || ''
  });
  ...
  useEffect(() => {
    if (mode === 'edit' && entity) {
      ...
      setFormData(p => ({ ...p, firmId: parentOpts?.firmId || '' }));
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: any = { ...formData, roleType: 'Member' };
      ...
      showToast('success', `Member ${mode === 'create' ? 'created' : 'updated'} successfully`);
      onSaved();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-xl font-bold text-gray-900">{mode === 'create' ? 'Add Member' : 'Edit Member'}</h3>
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <select value={formData.teamId} onChange={e => setFormData(p => ({ ...p, teamId: e.target.value }))} className="w-full px-4 py-3 bg-gray-50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 border border-transparent focus:border-emerald-300 transition-all">
              <option value="">No Team</option>
              {teams.filter((t: any) => t.managerId === formData.managerId || !formData.managerId).map((t: any) => (
